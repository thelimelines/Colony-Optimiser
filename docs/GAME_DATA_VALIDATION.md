...
- `toolsets.json` has nine toolsets with `key`, `usable`, and `useMultiplier`. Tool type behaviours use `id: tool`, `craftingspeed`, `durability`, optional `requiresItem`, and optional `scienceHint`. The installed server code converts `durability` from configured seconds to milliseconds, then decrements it by the effective tooled work time.
