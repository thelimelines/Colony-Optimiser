...
The acquisition service checks common Steam locations, Steam library folders, and the standard Steam path on every ready drive. On first run it enumerates `world.sqlite3` files below each discovered `gamedata\savegames` directory. A linked save is opened read-only and its `colonygroups` records are exposed as import scopes; the combined scope remains the default for compatibility, while multiplayer players can select one group. When the table exposes an explicit primary key, the app persists a hash of that key—supplemented by `creation_date` when that stable column is available—and validates it against the current save before changing progression. If the save exposes only SQLite's implicit `rowid`, the choice is usable for the current session but is not persisted, preventing a stale selection from silently resolving to a different group. If a saved stable group is missing or unreadable, progression is preserved and the app requires a new choice. Direct folder and file selection remain available. The upstream GitHub source ZIP is cached in `%LOCALAPPDATA%\ColonyOptimizer\GameData\GitHub`.
