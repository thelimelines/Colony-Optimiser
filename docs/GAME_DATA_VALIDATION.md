...
- Forestry uses the SimulationGraph forester action baseline of 390 seconds for nine trees, expressed as work seconds per tree. It is applied to the loaded worker-active interval and rounded down to complete trees: the current 444-second work interval produces ten whole trees per forester, subject to the physical 3-by-3 plot slots.
