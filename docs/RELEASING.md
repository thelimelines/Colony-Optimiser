...
6. Commit the version change and any lock-file changes caused by an intentional dependency update.
7. Create an annotated tag whose version exactly matches `Directory.Build.props`, then push it:
...
8. GitHub Actions checks out the required public game data, runs the test suite, publishes a self-contained Windows x64 portable ZIP, a per-machine MSI, and a Setup EXE. It then smoke-tests the finished Setup UI and embedded chain, administrative MSI deployment, and portable-ZIP contents before creating SHA-256 checksums and attaching the assets to the GitHub release. The app publish output includes Microsoft's small online WebView2 bootstrapper rather than the full standalone runtime; if visualisation initialisation fails, the app can use it to install or repair WebView2.
9. After the workflow succeeds, download the Setup EXE from the release page, verify its checksum using the instructions in `README.md`, and repeat the clean installation and first-use check as a final sanity check.
